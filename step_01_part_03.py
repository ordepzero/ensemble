...
def execute_svm_train(bases_of_samples,selected_channels,degree,conf,first_index,subdic_name):
    ...
    for samples in bases_of_samples:
        ...
        norm_base,mean,std = normalization(temp)
        norm_base[:,-1] = temp[:,-1]
        means.append(mean)
        stds.append(std)
        norm_bases.append(norm_base)
        
    #return create_train_tests_bases(norm_bases,means,stds,selected_channels,0)
                        ...
                        if(control_sct):
                            for cont_channel in range(n_total_channels):
                                ...
                                if(keep_selected_channels[cont_channel] == 0):
                                    ...
                                    norm_train,norm_test_temp,mean,std = create_train_tests_bases(norm_bases,means,stds,selected_channels,test_base_index)
...
if __name__ == "__main__": #VERSAO PARA EXECUTAR PARA 5 BASES
    ...
    subdic_name = "fc_5b_"+subdic_name[5:]+prefix
    for alg in l_algs:
