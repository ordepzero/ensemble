# -*- coding: utf-8 -*-
"""
Created on Tue Jul 10 09:39:47 2018
...
def execute(sel_chan_type,subject,filter_order,highcut,degree=2,part=0): 
    #print("Início")
    if(sel_chan_type == 0):
    ...
    n_signals_char = 180
    ciclo_size = 12
    n_all_char = n_bases * n_char_base
    ...
    for i in range(n_all_char):
        filename = config_e.create_sample_mat_file(conf,str(i)+"_train_")
        #print(filename)
        if(len(samples) == 0):
    ...
    for test_base_index in range(len(bases_of_samples)):
        ...
        cont_result_file = 0
        for C in c_values:
            print("C value Inicio")
            for coef0 in coef_values:
                for gamma in gamma_values:
                    
                    result_file = config_e.create_result_mat_file(conf,str(cont_result_file))
            ...
            for highcut in l_highcut:
                for sel_chan_type in l_sel_chan_type:
                    for degree in l_degree:
                        ...
                        execute(sel_chan_type,subject,filter_order,highcut,degree,0) # O 0 indica da sub base 0 a 8
                        execute(sel_chan_type,subject,filter_order,highcut,degree,1) # O 1 indica da sub base 9 a 17
