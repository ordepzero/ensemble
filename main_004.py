# -*- coding: utf-8 -*-
"""
Created on Sun Jul 22 16:19:51 2018
...
def load_all_samples(config_e,conf,n_all_char):
    samples = []
    for i in range(n_all_char):
        filename = config_e.create_sample_mat_file(conf,str(i)+"_train_dec_py_")
        #print(filename)
        if(len(samples) == 0):
...
def execute_svm_train(bases_of_samples,selected_channels,degree,conf,first_index):
        ...
        for C in c_values:
            for coef0 in coef_values:
                for gamma in gamma_values:
                    ...
                    result_file = config_e.create_result_mat_file(conf,str(cont_result_file)+"_")
                    pkl_filename = config_e.create_result_pkl_file(conf,test_base_index+first_index)
                    
                    cont_result_file = cont_result_file + 1
...
def execute(sel_chan_type,subject,filter_order,highcut,degree=2): 
    #print("Início")
    if(sel_chan_type == 0):
    ...
    samples = load_all_samples(config_e,conf,n_all_char)
    
    
    bases_of_samples = bases.create_sequential_bases(samples,n_bases)
    
    first_index = 8
    execute_svm_train(bases_of_samples[0:first_index],selected_channels,degree,conf,0)
    execute_svm_train(bases_of_samples[first_index:],selected_channels,degree,conf,first_index)
            ...
            for highcut in l_highcut:
                for sel_chan_type in l_sel_chan_type:
                    for degree in l_degree:
                        ...
                        execute(sel_chan_type,subject,filter_order,highcut,degree) # O 0 indica da sub base 0 a 8
